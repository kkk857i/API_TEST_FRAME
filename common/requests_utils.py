...
class RequestUtils():
    ...
    def __get(self,get_info):
        try:
            ...
            result={'code':4,'result':'[%s]请求:代理错误议常'%(get_info["接口名称"])}
        except ConnectionError as e:
            print(get_info["接口名称"])
            result={'code':4,'result':'[%s]请求:连接超时异常'%(get_info["接口名称"])}
        except RequestException as e:
            result={'code':4,'result':'[%s]请求:执行Request议常，原因是:%s'%(get_info["接口名称"],e.__str__())}
    ...
    def __post(self, post_info):
        try:
            ...
            result={'code':4,'result':'[%s]请求:代理错误议常'%(post_info["接口名称"])}
        except ConnectionError as e:
            result={'code':4,'result':'[%s]请求:连接超时异常'%(post_info["接口名称"])}
        except RequestException as e:
            result={'code':4,'result':'[%s]请求:执行Request议常，原因是:%s'%(post_info["接口名称"],e.__str__())}
        except Exception as e:
